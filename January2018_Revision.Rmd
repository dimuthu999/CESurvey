...
stargazer(merged_data[, c("interest_rate_cur","mtg_amount","non_durable","durable","services","total_mtg_pmt","total_exp","mtg_principal_outlay","org_year","interview_year","family_income_after_tax","family_size","age_of_ref")], type = output.type, summary.stat = c("mean", "sd", "p25", "median", "p75", "n"),notes = "",digits = 4)
```
```{r include=FALSE}
...
dep_vars <- c("non_durable","durable","services","principal_reduction","total_exp")
regs <- list()


regs[[1]] <- felm(as.formula(paste("I(",endo_var,"*1e5)~",instrument,"+",controls,"|0|",cluster_var,sep="")),data = reg_data)
i=2
...
```{r regs_main_cats,include=FALSE}
