...
merged_data <- datamor[datamor$interview_year-datamor$org_year<=2,]
...
merged_data <- merge(merged_data,datafmli[,c("CUID","state","total_exp")],by="CUID",all.x = TRUE)
...
merged_data['exp'] <- merged_data$total_exp - merged_data$mtg_pmt_calc_mor

merged_data <- merged_data[merged_data$exp >0,]

dependent_var = "log(exp)"
endo_var = "mpay_mtg"
instrument = "mtg30us"
cluster_var = "interview_year+state"

controls <- "0|org_year+interview_year+state"



regs <- list()
regs[[1]] <- felm(as.formula(paste(dependent_var,"~",endo_var,"+",controls,"|0|",cluster_var,sep="")),data = merged_data)
regs[[2]] <- felm(as.formula(paste(endo_var,"~",instrument,"+",controls,"|0|",cluster_var,sep="")),data = merged_data)
regs[[3]] <- felm(as.formula(paste(dependent_var,"~",controls,"|(",endo_var,"~",instrument,")|",cluster_var,sep="")),data = merged_data)
